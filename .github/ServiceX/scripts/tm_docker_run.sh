...
tm_docker_run() {
    ...
    local usage="$FUNCNAME [-h] [-e \"entrypoint\"] [-i <image>] [-l <loacal path to mount>] [-R <registry>] [-r <remote path to mount>]
	...
	-n [container name]   to keep the contain after this run, specify the container name (default: ${name})
    ...
    if [[ "${globus}" == "true" ]]; then
	cmd="${cmd} -v ~/.globus:/home/cmsuser/.globus"
    fi
