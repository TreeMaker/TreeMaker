...
#all .jdl files are named jobExecCondorNeff_JOBNAME.jdl
...
for file in $(grep -lw "return value\|abort" $(find *.condor -newermt "${TIME}")); do
	...
	if [ -z "$newerstatus" ]; then
		...
		echo "condor_submit jobExecCondorNeff_${base}.jdl" >> ${OUTNAME}
	fi
