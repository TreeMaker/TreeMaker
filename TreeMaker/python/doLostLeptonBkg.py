...
def doLostLeptonBkg(process,geninfo):
    ...
    process.TreeMaker2.VarsRecoCand.extend(['IsolatedElectronTracksVeto','IsolatedMuonTracksVeto','IsolatedPionTracksVeto'])
    process.TreeMaker2.VarsRecoCand.extend(['LeptonsNew:IdIsoMuon(selectedIDIsoMuons)','LeptonsNew:IdMuon(selectedIDMuons)','LeptonsNew:IdIsoElectron(selectedIDIsoElectrons)','LeptonsNew:IdElectron(selectedIDElectrons)','SelectedPFCandidates'])
    ...
    if geninfo: # gen information on leptons
        process.TreeMaker2.VarsRecoCand.extend(['GenLeptons:Boson(GenBoson)','GenLeptons:TauDecayCands(TauDecayCands)','GenLeptons:TauNu(GenTauNu)'])
    ...
    process.TreeMaker2.VarsRecoCand.extend(['LeptonsNewTag:IdIsoMuon(selectedIDIsoMuonsNoMiniIso)','LeptonsNewTag:IdIsoElectron(selectedIDIsoElectronsNoMiniIso)'] )
    ...
    process.TreeMaker2.VarsRecoCand.extend(['slimmedElectrons','slimmedMuons'])
    process.TreeMaker2.VarsRecoCand.extend(['SelectedPFElecCandidates','SelectedPFMuCandidates','SelectedPFPionCandidates'])
