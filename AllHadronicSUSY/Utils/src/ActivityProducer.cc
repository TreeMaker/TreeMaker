...
private:
	...
	virtual void endJob() override;
        ...
        edm::InputTag objectSource_, objectMatchSource_, WeightTag_;
...
ActivityProducer::ActivityProducer(const edm::ParameterSet& iConfig)
{
	...
	jetSrc_ = iConfig.getParameter<edm::InputTag>("jetSrc");
        WeightTag_ = iConfig.getParameter<edm::InputTag>("WeightTag");
...
// 	produces<edm::ValueMap<float> >("miniIso");
	...
	produces<edm::ValueMap<float> >("MTW");
        produces<edm::ValueMap<float> >("MTWClean");
...
ActivityProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	reco::MET::LorentzVector metLorentz(0,0,0,0);
        reco::MET::LorentzVector metLorentzclean(0,0,0,0);
...
// 	    std::cout<<"ActivityProducer::Warning the input collection size to recompute MET to emulate W decays is not 1, this value will always pick up the first object in this collection. It now met will not be recomputed"<<TagObjectTag_.label()<<"\n";
	    if(tagObject->size()>0)
            {
              metLorentz += tagObject->at(0).p4();
              metLorentzclean += tagObject->at(0).p4();
            }
	...
	// muon starting with muon as probe
	if(objectTyp_==1)
	{
	  ...
	  tagObjectSize.push_back(size );
			...
			float passingTemp=0;
			reco::MET::LorentzVector metLorentz2(0,0,0,0);
                        reco::MET::LorentzVector metLorentz2clean(0,0,0,0);
			for(unsigned int ii=0; ii< objectMatch->size();ii++)
			{
				if(deltaR(object->at(i).eta(),object->at(i).phi(),objectMatch->at(ii).eta(),objectMatch->at(ii).phi() ) < 0.07&& std::abs(object->at(i).pt() - objectMatch->at(ii).pt() )/objectMatch->at(ii).pt() < 0.4)
				{
					...
					if(objectMatch->size()==2 && objectMatch->size()== tagObject->size())
					{
						
						metLorentz2=MET->at(0).p4();
                                                if(ii==0)	
                                                {
                                                  metLorentz2+= objectMatch->at(1).p4();
                                                  metLorentz2clean+= objectMatch->at(1).p4();
                                                }
                                                else if(ii==1) 
                                                {
                                                  metLorentz2+= objectMatch->at(0).p4();
                                                  metLorentz2clean+= objectMatch->at(1).p4();
                                                }
					}
					else 
                                        {
                                          metLorentz2 = reco::MET::LorentzVector(-10000,0,0,0);
                                          metLorentz2clean+= reco::MET::LorentzVector(-10000,0,0,0);
                                        }
				}
				
			}
			newMet.push_back(metLorentz2.pt());
			mtw.push_back(MTWCalculator(metLorentz2.pt(),metLorentz2.phi(),object->at(i).pt(),object->at(i).phi()));
                        mtw_clean.push_back(MTWCalculator(metLorentz2clean.pt(),metLorentz2clean.phi(),object->at(i).pt(),object->at(i).phi()));
...
// 		std::cout<<"Amount of object: "<<object->size()<<" amount of activity variables in vector to be stored: "<<values.size()<<std::endl;
		...
		iEvent.put(valMap7, "MTW");
                ...
                filler8.insert(object, mtw_clean.begin(), mtw_clean.end());
                filler8.fill();
                iEvent.put(valMap8, "MTWClean");
...
// 		std::cout<<"Event Dont!"<<std::endl;
	...
	// muon starting with packedCandidates
...
// 		std::cout<<"Event Dont!"<<std::endl;
	...
	// electron starting with electron as probe
...
//                             probePassingCandidate_.push_back(object->at(i));
	...
	// electron starting with photon as probe
...
//                             probePassingCandidate_.push_back(object->at(i));
	...
	// isolated track muon starting with packedCandidates as probe
...
// 						activity+= jets->at(ii).pt() * (jets->at(ii).chargedHadronEnergyFraction() + jets->at(ii).chargedEmEnergyFraction() );
			...
			newMet.push_back(metLorentz.pt());
			mtw.push_back(MTWCalculator(metLorentz.pt(),metLorentz.phi(),object->at(i).pt(),object->at(i).phi()));
                        mtw_clean.push_back(MTWCalculator(metLorentzclean.pt(),metLorentzclean.phi(),object->at(i).pt(),object->at(i).phi()));
			int size=tagObject->size();
	    tagObjectSize.push_back(size );
			// check if the probe object can be matched to an object from the objectMatchSource collection if so passing if not failing
			float passingTemp=0;
			for(unsigned int ii=0; ii< objectMatch->size();ii++)
			{
				if(deltaR(object->at(i).eta(),object->at(i).phi(),objectMatch->at(ii).eta(),objectMatch->at(ii).phi() ) < 0.07&& std::abs(object->at(i).pt() - objectMatch->at(ii).pt() )/objectMatch->at(ii).pt() < 0.2)
				{
					if(tagObject->size()==1)
					{
// 						std::cout<<"tagObjectsize: 1 matched to probe object will it be skipped?... with deltaR: "<<deltaR(object->at(i).eta(),object->at(i).phi(),tagObject->at(0).eta(),tagObject->at(0).phi() ) <<"deltaPT: "<<std::abs(object->at(i).pt() - tagObject->at(0).pt() )/tagObject->at(0).pt() <<std::endl;
...
// std::cout<<"tagObjectsize: 1 matched to probe object will be skipped... with deltaR: "<<deltaR(object->at(i).eta(),object->at(i).phi(),tagObject->at(0).eta(),tagObject->at(0).phi() ) <<"deltaPT: "<<std::abs(object->at(i).pt() - tagObject->at(0).pt() )/tagObject->at(0).pt() <<std::endl;
							continue;
						}
					}
					passingTemp++;
...
// 			if(tagObject->size()>1)
// 			{
// 				std::cout<<"mu iso track for 2 tag objects passingTemp: "<<passingTemp<<std::endl;
// 			}
			passing.push_back(passingTemp);
	...
	// isolated track electron starting with packedCandidates as probe
...
// 						activity+= jets->at(ii).pt() * (jets->at(ii).chargedHadronEnergyFraction() );
	...
	// isolated track pion starting with packedCandidates as probe
	else if(objectTyp_==5)
        {
          //              edm::Handle<std::vector<pat::Muon> > object;
          //              iEvent.getByLabel(objectSource_, object);
          edm::Handle<edm::View< pat::PackedCandidate> >object;
          ...
          for(unsigned int i=0; i<object->size();i++)
          {
            ...
            for(unsigned int ii=0; ii<jets->size();ii++)
            {
              if( deltaR(object->at(i).eta(),object->at(i).phi(),jets->at(ii).eta(),jets->at(ii).phi()) < maxDeltaR_)
              {
                if(activityTyp_==0)
                {
                  activity+= jets->at(ii).pt() * (jets->at(ii).neutralEmEnergyFraction() + jets->at(ii).photonEnergyFraction());
            ...
            passing.push_back(passingTemp);
            if(passingTemp>1) std::cout<<"Typ5::Warning 2 have been matched!!!"<<std::endl;
          }
          //              std::cout<<"Amount of object: "<<object->size()<<" amount of activity variables in vector to be stored: "<<values.size()<<std::endl;
          std::auto_ptr<ValueMap<float> > valMap2(new ValueMap<float>());
          ValueMap<float>::Filler filler2(*valMap2);
