...
* `jetsconstituents`: store all constituents from all jet collections; each jet collection gets an index branch into the single vector of all constituents; options: 0 = don't store, 1 = store 4-vector and PDG ID, 2 = also store other properties (default=0)
